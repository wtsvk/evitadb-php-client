...
This is a thin PHP gRPC client for EvitaDB. The library is two hand-written files plus a large generated tree:

- **`src/EvitaDbClientInterface.php`** — the contract consumers type-hint against. Enables mocking in consumer tests without depending on the concrete implementation.
- **`src/EvitaDbClient.php`** — the concrete implementation. Receives `EvitaServiceClient` and `EvitaSessionServiceClient` via constructor injection (pure DI). A static `EvitaDbClient::create(host, port, grpcOpts)` factory is provided for convenience when no DI container is available. Every public method (`query`, `getEntity`, `upsertEntity`, `defineEntitySchema`, `deleteEntity`) opens a fresh gRPC session, runs the call, and closes the session in a `finally` block. The class holds no per-call state; it's safe for long-running runtimes (Laravel Octane, RoadRunner). Sessions come in two flavors via `withReadSession()` / `withWriteSession()`, which wrap `EvitaService.CreateReadOnlySession` / `CreateReadWriteSession` and pass the resulting session ID as the `sessionid` gRPC metadata header. Errors are thrown (`EvitaDbConnectionException` for transport failures, `EvitaDbStatusException` for non-OK gRPC status); `closeSession` intentionally swallows exceptions because it runs in `finally`.
...
- **`src/Exception/`** — hierarchy: abstract base `EvitaDbException` extends `RuntimeException`; concrete subclasses are `EvitaDbConnectionException` (transport failures), `EvitaDbStatusException` (non-OK gRPC status), and `EvitaDbEntityNotFoundException`.
...
- **`@template` generics** on interfaces and methods that operate on variable types. The existing `withReadSession`/`withWriteSession` already use this pattern.
